...
impl<T: Clone + Copy + Num + Zero> DelayLine<T> {
    ...
    fn left_update(&mut self, v_left_minus: T) {
        self.v_left_minus = v_left_minus;
        self.v_left_minus_history.push(v_left_minus);
    }

    fn right_update(&mut self, v_right_minus: T) {
...
struct String<T> {
    delay_line_left: DelayLine<T>,
    delay_line_right: DelayLine<T>,
}

impl<T: Clone + Copy + Num + Zero> String<T> {
